...
# 👋🏻 프로젝트 소개
<p>Pinoco는 친구들과 실시간으로 라이어 게임을 즐길 수 있는 화상 채팅 기반의 웹 애플리케이션입니다.</p>

웹 상에서 P2P로 미디어를 공유하는 기술인 WebRTC를 활용하여 실시간으로 화상으로 진행하는 라이어 게임을 구현했습니다.

심리전이 중요한 게임인 라이어 게임의 재미를 극대화 할 수 있었습니다.
...
각 서버의 용도에 따라 두 개의 인스턴스로 분리하여 운영 중입니다.

Socket.io를 통한 소켓 연결과 게임 로직을 관리하는 게임 서버를 관리하는 인스턴스와

WebRTC 연결을 위한 시그널링, STUN/TURN을 담당하는 서버가 운영되는 인스턴스로 분리하였습니다.

각 서버의 부하를 분리하여 안정성을 높였고, 서버 부하를 명확하게 파악하여 스케일링이 가능한 구조를 고려했습니다.
